const places = [
  {
    ...
    category: ["local_chinese", "fine_dining"],
  ...
  {
    name: "Jiin Omakase",
    category: ["fine_dining", "michelin", "japan_korea"],
  ...
  {
    name: "ASIN",
    category: ["fine_dining", "local_chinese"],
        ...
        <article class="place-card">
          ...
          <div class="place-body">
            <div class="place-head">
              <div>
                <p class="card-kicker">${place.typeLabel}</p>
                <h3>${place.name}</h3>
                <div class="meta">
                  <span>${place.area}</span>
                  <span>开业 ${place.opened}</span>
                  <span>${place.spend}</span>
                </div>
              </div>
            </div>
            ...
            <div class="shoot-tags">
              <span>适合拍</span>
              ${place.shootFor.map((tag) => `<b>${tag}</b>`).join("")}
            </div>
            <p class="take">${place.why}</p>
            <div class="prediction-box">
              <span>AI 预测</span>
              <p>${place.prediction}</p>
            </div>
            <div class="angle-box">
              <span>小红书选题</span>
              <p>${place.angle}</p>
            </div>
            <p class="proof">${place.proof}</p>
            <div class="card-actions">
              <a href="${place.source}" target="_blank">看来源</a>
              <button type="button" data-save="${place.name}">收藏</button>
            </div>
          </div>
        </article>
      `
...
refreshFeed.addEventListener("click", () => {
  ...
  renderPlaces(currentFilter);
  window.setTimeout(() => refreshFeed.classList.remove("spinning"), 500);
});
